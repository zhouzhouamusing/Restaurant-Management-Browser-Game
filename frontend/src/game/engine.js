...
export class GameEngine {
  ...
  _handleStaffResult(result) {
    if (result.type === "checkout_done") {
      ...
      this._spawnServeSuccessEffect(
        result.customer.x,
        result.customer.y - 20,
        result.customer.orderedDish,
      );
      this._spawnStaffEffect(
        result.customer.x,
        result.customer.y - 40,
        result.staff,
      );
  ...
  _spawnServeSuccessEffect(x, y, dish) {
    ...
    // Dish emoji floating up with checkmark
    this.floatingTexts.push({
      x,
      y: y - 25,
      text: `${dish?.emoji || "🍲"} ✓`,
  ...
  _drawCustomers(ctx) {
    for (const c of this.customers) {
      ...
      if (
        c._beingServedBy &&
        c._beingServedBy.busy &&
        !c._beingServedBy.moving
      ) {
  ...
  _drawPatienceBar(ctx, x, y, patience) {
    ...
    const color =
      patience > 60 ? "#2ecc71" : patience > 30 ? "#f39c12" : "#e74c3c";
    ctx.beginPath();
  ...
  _drawStaffCharacters(ctx) {
    for (const s of this.staff) {
      ...
      const walkBob = isMoving
        ? Math.sin(s.walkFrame * 0.15) * 2
        : Math.sin(s.animPhase) * 1;
      const sx = s.x;
  ...
  _drawFloatingTexts(ctx) {
    for (const t of this.floatingTexts) {
      ...
      // Draw coin icon next to coin amounts
      if (t.coinIcon) {
  ...
  _drawCashierCounter(ctx, w, h) {
    ...
    // Draw cash register (canvas drawn, not emoji)
  ...
  _drawCashRegister(ctx, x, y) {
    ...
    // Coin particles on checkout animation
